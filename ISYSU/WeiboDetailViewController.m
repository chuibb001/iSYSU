...
-(void)initEmotions{
    ...
    tooEmotion=[[UIButton alloc] initWithFrame:CGRectMake(260, -40, 44, 44)];
    //tooEmotion=[[UIButton alloc] initWithFrame:CGRectMake(260, -32.5, 21, 21)];
...
-(void)initNotifications{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didSendCommentSuccess) name:@"weibo_sendcomment" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didSendCommentFail) name:@"weibo_sendcommentFail" object:nil];
...
-(void)send{
    if ([self.content.text isEqualToString:@""]) {
        ...
        if (length>140) {
            ...
            [[SinaWeiboData shared] sendComment:self.aID withComment:self.content.text];
        }
    }
}
